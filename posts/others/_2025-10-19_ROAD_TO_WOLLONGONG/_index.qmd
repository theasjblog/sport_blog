---
title: "Road to Wollongong: 2025 training season recap"
...
all_files <- lapply(list.files(file.path(
  '2025_season'
  ),
...
df <- all_files |>
  ...
  summarise(tot_n_session = sum(n_activities, na.rm = TRUE),
            avg_n_session_week = round(sum(n_activities, na.rm = TRUE)/max(week_n), 1),
            tot_h = hms::as_hms(sum(moving_time, na.rm = TRUE)),
...
df <- all_files |>
  ...
  summarise(tot_n_session = sum(n_activities, na.rm = TRUE),
            avg_n_session_week = round(mean(n_activities, na.rm = TRUE), 1),
            tot_h = hms::as_hms(sum(moving_time, na.rm = TRUE)),
            tot_distance = sum(total_distance_km, na.rm = TRUE))
kableExtra::kable(df)
```
