...
race_metadata <- arrow::open_dataset('../../../races_metadata.parquet') |>
  filter(EVENT_DATE == as.Date("2025-05-04") | EVENT_DATE == as.Date("2025-04-12") | EVENT_DATE == as.Date("2025-02-22")) |>
  collect()



all_races <- arrow::open_dataset('../../../races_rankings.parquet') |>
  filter(RACE_ID %in% race_metadata$RACE_ID) |>
  collect()

activities_metadata <- arrow::open_dataset('../../../activities_metadata.parquet') |>
  filter(RACE_ID %in% race_metadata$RACE_ID) |>
  collect()

activities_data <- arrow::open_dataset('../../../activities_data.parquet') |>
  filter(ID %in% activities_metadata$ID) |>
  collect()
