...
description: "The final indoor race and series recap, with the age-group result settled and the full progression across the four-race block."
...
analysis_data <- get_post_analysis_data(c('2025-05-04', '2025-04-12', '2025-02-22'))
...
summary_runs <- all_runs |>
  ...
  summarise(tot_distance = round(max(KM, na.rm = TRUE), 2),
            ...
            avg_cadence = round(mean(CAD, na.rm = TRUE)),
            cadence_var = round(100* (mean(CAD[floor(n()/2)+1:n()], na.rm = TRUE) - mean(CAD[1:floor(n()/2)], na.rm = TRUE)) / avg_cadence, 2),
